...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    /**
     * 首先遍历ServiceBean的List registries（所有注册中心的配置信息），然后将地址封装成URL对象，关于注册中心的所有配置属性，最终转换成url的属性(?属性名=属性值)，loadRegistries(true)，
     * 参数的意思：true，代表服务提供者，false：代表服务消费者，如果是服务提供者，则检测注册中心的配置，
     * 如果配置了register=”false”，则忽略该地址，如果是服务消费者，并配置了subscribe=”false”则表示不从该注册中心订阅服务，故也不返回，一个注册中心URL示例：
    ...
    protected List<URL> loadRegistries(boolean provider) {
        // <dubbo:registry address="zookeeper://172.16.11.118:2181" id="com.alibaba.dubbo.config.RegistryConfig" /> 检测注册中心 并为注册中心赋默认值
        checkRegistry();
        ...
        if (registries != null && registries.size() > 0) {
            for (RegistryConfig config : registries) {
                ...
                if (address != null && address.length() > 0
                    ...
                    // map中 key-val为默认值, address中值为基础值覆盖map中默认属性后，组成URL信息 ， 信息为注册中心的信息 如 protocal=zookeeper host=ip(11.118) 端口2181等  path=com.alibaba.dubbo.registry.RegistryService 和 没覆盖的属性放在parameters中
                    //zookeeper://172.16.11.118:2181/com.alibaba.dubbo.registry.RegistryService?application=demo-provider&dubbo=2.0.0&pid=12336&timestamp=1527858600166
                    List<URL> urls = UrlUtils.parseURLs(address, map);
                    for (URL url : urls) {
                        url = url.addParameter(Constants.REGISTRY_KEY, url.getProtocol());
                        url = url.setProtocol(Constants.REGISTRY_PROTOCOL);
                        // registry://172.16.11.118:2181/com.alibaba.dubbo.registry.RegistryService?application=demo-provider&dubbo=2.0.0&pid=9612&registry=zookeeper&timestamp=1527859062297
